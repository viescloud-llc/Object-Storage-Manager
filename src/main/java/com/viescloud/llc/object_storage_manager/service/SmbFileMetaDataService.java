...
@Service
@Slf4j
public class SmbFileMetaDataService extends ViesServiceWithUserAccess<SmbFileMetaData, Integer, SmbFileMetaDataDao> {
    ...
    @Override
    @Deprecated
    public List<SmbFileMetaData> getAll() {
        throw new UnsupportedOperationException("method getAll() should not be use");
    }

    public List<SmbFileMetaData> getAll(int userId) {
        return this.repositoryDao.findAllByUserId("/" + userId + "/%");
    }

    public SmbFileMetaData getFileMetaDataById(int id) {
    ...
    public SmbFileMetaData getFileMetaDataByPath(String path) {
    ...
    private SmbFileMetaData getFileMetaDataByPathWithTry(String path, int userId, int numTry) {
        if (numTry >= 20)
            return (SmbFileMetaData) HttpResponseThrowers
    ...
    public SmbFileMetaData getFileMetaDataByPath(String path, int userId) {
        return getFileMetaDataByPathWithTry(path, userId, 0);
    }

    public SmbFileMetaData getFileMetaDataByName(String fileName, int userId) {
        return this.getFileMetaDataByPath(String.format("/%s/%s", userId, fileName), userId);
    }

    public SmbFileMetaData getFileByPath(String path) {
        var metadata = this.getFileMetaDataByPath(path);
        return ObjectUtils.isEmpty(metadata) ? null : this.processingGetOutput(metadata);
    }

    public SmbFileMetaData getFileByPath(String path, int userId) {
        return getFileByPathWithTry(path, userId, 0);
    }

    private SmbFileMetaData getFileByPathWithTry(String path, int userId, int numTry) {
        var metadata = this.getFileMetaDataByPathWithTry(path, userId, numTry);
        return ObjectUtils.isEmpty(metadata) ? null : this.processingGetOutput(metadata);
    }

    public SmbFileMetaData getFileByName(String fileName, int userId) {
        return this.getFileByPath(String.format("/%s/%s", userId, fileName), userId);
    }

    public SmbFileMetaData getFileMetaDataByCriteria(String id, String path, String fileName, int userId) {
    ...
    public SmbFileMetaData getFileByCriteria(String id, String path, String fileName, int userId) {
        var metadata = this.getFileMetaDataByCriteria(id, path, fileName, userId);
        return ObjectUtils.isEmpty(metadata) ? null : this.processingGetOutput(metadata);
    }

    public SmbFileMetaData patchFileMetaData(SmbFileMetaData originalMetadata, SmbFileMetaData newMetaData) {
    ...
    public boolean isFileExist(String path) {
        try {
            ...
            if (exist) {
                ...
                var metadata = SmbFileMetaData.builder().publicity(false).contentType(contentType).originalFilename(fileName).path(path).size(size).ownerUserId(userId).build();
    ...
    @Override
    public boolean isRelatedToUser(SmbFileMetaData fileMetaData, int userId) {
    ...
    public byte[] getData(SmbFileMetaData fileMetaData) {
        InputStream raw;
        byte[] byteArray = null;
    ...
    @Override
    protected SmbFileMetaData processingGetOutput(SmbFileMetaData object) {
    ...
    @Override
    protected SmbFileMetaData processingPostInput(SmbFileMetaData object) {
    ...
    @Override
    protected SmbFileMetaData processingPostOutput(SmbFileMetaData object) {
        try {
