...
public class SmbFileManagerController {
    ...
    @Autowired
    private SmbFileMetaDataService fileMetaDataService;
    ...
    private void resizeImage(SmbFileMetaData metadata, ImageFormats imageFormat, int width, int height) {
    ...
    private void resizeVideo(SmbFileMetaData metadata, VideoFormats videoFormat, int width, int height) {
        IVSize customRes = new IVSize();
        customRes.setWidth(width);
        customRes.setHeight(height);
    ...
    public List<SmbFileMetaData> getMetadata(@RequestHeader(required = false) String user_id) {
    ...
    @GetMapping("metadata")
    public SmbFileMetaData getMetadata(
            @RequestHeader(required = false) String user_id,
            @RequestParam(required = false) String path,
            @RequestParam(required = false) String fileName,
            @RequestParam(required = false) String id) {
        ...
        if (!ObjectUtils.isEmpty(metadata))
            ...
            return (SmbFileMetaData) HttpResponseThrowers.throwNotFound("Metadata not found");
    }

    @PostMapping("file")
    public SmbFileMetaData uploadFile(
        @RequestHeader(required = false) String user_id, 
        ...
        if(publicity == null)
            publicity = false;
        int userId = Integer.parseInt(user_id);
        var metadata = SmbFileMetaData.fromMultipartFile(file, userId, file.getBytes(), publicity);
        return this.fileMetaDataService.post(metadata);
    }

    @PatchMapping("metadata")
    public SmbFileMetaData patchMetaData(
            @RequestHeader(required = false) String user_id,
            @RequestParam(required = false) String path,
            @RequestParam(required = false) String fileName,
            @RequestParam(required = false) String id,
            @RequestBody SmbFileMetaData fileMetaData) {
        if(ObjectUtils.isEmpty(user_id))
            HttpResponseThrowers.throwUnauthorized("Unauthorized");
        int userId = Integer.parseInt(user_id);
        var metadata = this.fileMetaDataService.getFileMetaDataByCriteria(id, path, fileName, userId);
